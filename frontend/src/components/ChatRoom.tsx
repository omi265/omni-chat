...
export default function ChatRoom({ roomId, username, joinedServers, onSwitchServer, onLeaveHome }: ChatRoomProps) {
  ...
  }, [messages, isVoiceView, roomFiles]);
  ...
  return (
    <div className="flex h-full bg-[#313338] text-gray-200 font-sans overflow-hidden relative">
      ...
      {/* 2. Sidebar (Includes Rail on Mobile) */}
      <aside className={`
      ...
      `}>
        {/* Mobile Server Rail (Integrated into sidebar on mobile) */}
        ...
        <div className="h-12 border-b border-[#1E1F22] px-4 flex items-center justify-between shadow-sm font-bold text-white uppercase text-[11px] tracking-wider transition-colors hover:bg-[#35373C] cursor-pointer group">
          ...
          <div className="flex items-center space-x-1">
            <button onClick={handleInvite} className="text-[#949ba4] hover:text-white p-1" title="Invite People" aria-label="Invite People">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" /></svg>
            </button>
            <button onClick={() => setIsSidebarOpen(false)} className="md:hidden text-gray-400 p-1" aria-label="Close Sidebar">
        ...
        <nav className="flex-1 p-2 space-y-2 overflow-y-auto custom-scrollbar text-left">
          ...
          <section>
            <div className="px-2 mb-1 flex items-center justify-between text-[11px] font-semibold text-[#949ba4] uppercase tracking-wider group cursor-pointer">
              ...
              <button onClick={() => setIsCreatingChannel({ type: 'text' })} className="text-gray-400 hover:text-gray-200 text-lg leading-none" aria-label="Create Text Channel">+</button>
            </div>
            {expandedSections.text && (
              <div className="space-y-0.5">
                {channels.filter(c => c.type === 'text').map(channel => (
                  <div 
                    key={channel.id} 
                    onClick={() => { switchChannel(channel.id); setIsSidebarOpen(false); }} 
                    className={activeChannelId === channel.id ? "px-2 py-1.5 rounded bg-[#3F4147] text-white flex items-center space-x-2 cursor-pointer group" : "px-2 py-1.5 rounded hover:bg-[#35373C] text-[#949ba4] flex items-center space-x-2 cursor-pointer group"}
                  >
                    ...
                    {channel.id && !channel.id.endsWith(':general') && (
                      <button onClick={(e) => { e.stopPropagation(); deleteChannel(channel.id); }} className="opacity-0 group-hover:opacity-100 text-xs text-gray-500 hover:text-red-400" aria-label={`Delete channel ${channel.name}`}>×</button>
                    )}
                  </div>
                ))}
              </div>
          ...
          <section>
            <div className="px-2 mt-4 mb-1 flex items-center justify-between text-[11px] font-semibold text-[#949ba4] uppercase tracking-wider group cursor-pointer">
              ...
              <button onClick={() => setIsCreatingChannel({ type: 'voice' })} className="text-gray-400 hover:text-gray-200 text-lg leading-none" aria-label="Create Voice Channel">+</button>
            </div>
              ...
              <div className="space-y-1">
                {channels.filter(c => c.type === 'voice').map(channel => {
                  ...
                  return (
                    <div key={channel.id}>
                      <div 
                        onClick={() => { handleJoinVoice(channel.id); switchChannel(channel.id); if(!isJoined) setIsSidebarOpen(false); }} 
                        className={(activeChannelId === channel.id) ? "px-2 py-1.5 rounded bg-[#3F4147] text-white flex items-center space-x-2 cursor-pointer transition group" : "px-2 py-1.5 rounded hover:bg-[#35373C] text-[#949ba4] flex items-center space-x-2 cursor-pointer transition group"}
                      >
                        ...
                        {channel.id && !channel.id.endsWith(':voice-gen') && (
                          <button onClick={(e) => { e.stopPropagation(); deleteChannel(channel.id); }} className="opacity-0 group-hover:opacity-100 text-xs text-gray-500 hover:text-red-400" aria-label={`Delete voice channel ${channel.name}`}>×</button>
                        )}
                      </div>
                      {isJoined && (
                        <div className="ml-8 space-y-1 border-l border-[#3F4147] pl-2 mt-1">
                          {roomMembers.filter(m => m.isOnline && m.voiceChannelId === channel.id).map(m => (
                            <div key={m.username} className="text-xs py-1 flex items-center">
                              <div 
                                className={`w-5 h-5 rounded-full flex items-center justify-center text-white text-[8px] font-bold border-2 mr-2 overflow-hidden ${m.isSpeaking ? 'border-[#23a559]' : 'border-transparent'}`} 
                                style={{ backgroundColor: m.avatarColor || '#5865F2' }}
                              >
        ...
        <div className="bg-[#232428] p-2 flex flex-col shrink-0">
          ...
          <div className="h-[52px] px-2 flex items-center space-x-2 text-left">
            <div className="relative flex-shrink-0">
              ...
              <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-[#232428] rounded-full flex items-center justify-center"><div className="w-2 h-2 bg-[#23a559] rounded-full" /></div>
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-white text-xs font-bold truncate">{username}</div>
              <div className="text-[#949ba4] text-[10px]">Online</div>
            </div>
            <button onClick={() => setShowSettings(true)} className="p-1.5 hover:bg-[#3F4147] rounded-md text-[#949ba4] hover:text-white transition" aria-label="User Settings">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#313338]">
        ...
        {isVoiceView ? (
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div ref={scrollRef} className="flex-1 overflow-y-auto custom-scrollbar text-left">
              <div className="flex flex-col justify-end min-h-full pb-4">
                <div className="py-12 px-4 mb-4 border-b border-[#3F4147]/20 text-center">
                  ...
                  <p className="text-[#b5bac1] text-sm px-4">Everything here is end-to-end encrypted.</p>
                </div>
            ...
            <div className="p-4 bg-[#313338]">
              <div className="bg-[#383A40] rounded-lg px-4 flex items-center shadow-inner">
                <button onClick={() => fileInputRef.current?.click()} className="text-[#b5bac1] hover:text-[#dbdee1] p-2 mr-2 transition-colors" aria-label="Attach File"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg></button>
      ...
      {isCreatingChannel && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 text-left font-sans" role="dialog" aria-labelledby="create-channel-title">
          <div className="bg-[#313338] w-full max-w-md rounded-lg p-6 shadow-2xl border border-[#1E1F22]">
            <h2 id="create-channel-title" className="text-2xl font-bold text-white mb-2">Create {isCreatingChannel.type === 'text' ? 'Text' : 'Voice'} Channel</h2>
      ...
      {showServerInfo && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 font-sans text-white text-left" role="dialog" aria-labelledby="server-info-title">
          <div className="bg-[#313338] w-full max-w-md rounded-lg p-6 shadow-2xl border border-[#1E1F22]">
            <h2 id="server-info-title" className="text-2xl font-bold text-white mb-4">Server Settings</h2>
